...
from convert_to_narziss.converttonarziss import NexusFile, ScanDataFile, ScanDataReader, get_data
from convert_to_narziss.orso import Person, Measurement, InstrumentSettings, Experiment
...
class TestOrsoModule:
    @pytest.mark.skip(reason="auto_fill decorator incompatible with orsopy base classes")
    def test_auto_fill_measurement(self):
        meas = Measurement(data_files=["file.dat"])
        assert meas.scheme == "angle-dispersive"
    
    @pytest.mark.skip(reason="auto_fill decorator incompatible with orsopy base classes")    
    def test_auto_fill_instrument_settings(self):
    ...
    @pytest.mark.skip(reason="auto_fill decorator incompatible with orsopy base classes")
    def test_auto_fill_experiment(self):
        exp = Experiment(title="Test")
        assert exp.probe == "neutron"
        assert exp.facility == "BNC"
    
    def test_auto_fill_person(self):
        person = Person(name="Test User")
        assert person.affiliation == "BNC user"
...
class TestNexusFile:
    ...
    def test_flippers_data(self, nexus_file):
    ...
    def test_instrument_settings(self, nexus_file):
        ...
        assert isinstance(result, InstrumentSettings)
